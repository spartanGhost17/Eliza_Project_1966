    ...
    def load(self, path):
        ...
        with open(path) as file:
            for line in file:
                ...
                if not line.strip():
                    continue
                # split every line at : and loop over all indicies then strip front and back of index value
                tag, content = [part.strip() for part in line.split(':')]
                # check tag and append stripped values in to respective dictionnaries
                # initial tag = conversation starter
                if tag == 'initial':
                    self.initials.append(content)
                # final tag = conversation ender    
                elif tag == 'final':
                    self.finals.append(content)
                # quit tag = cut conversation short    
                elif tag == 'quit':
                    self.quits.append(content)
                # pre tag = pre transformation of input key word in recomposition phase  
                elif tag == 'pre':
                    parts = content.split(' ')
